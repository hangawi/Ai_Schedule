  ...
  methods: {
    ...
    handleTouchEnd: function handleTouchEnd(event) {
      if (this.canMove) {
        ...
        var action = difference > 200 ? 'moveNextBoard' : 'movePrevBoard';

        if (Math.abs(difference) > this.mdSwipeDistance) {
          this[action]();
          
          console.log('vue ' + action);
          _direction_of_move = action;
          var _target = getTarget();

          // console.log(_target)

          // console.log('touchend' + _direction_of_move + ' ' + _stored_list.length)
          
          if(_direction_of_move === 'moveNextBoard' && _target < _stored_list.length - 1) {
            _target++;
            // $$main.currentWord = _target;
          } 
          if(_direction_of_move === 'movePrevBoard' && _target > 0) {
            _target--;
            // $$main.currentWord = _target;
          }
          swipeTarget(_target);
