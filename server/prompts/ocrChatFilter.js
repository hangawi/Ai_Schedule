...
분석:
- "댄스 학원은 공연반만" → title에 "댄스" 또는 "KPOP" 포함 AND "공연반" 포함
- "필라테스는 7시 이후만" → title에 "필라테스" 관련 키워드 포함 AND startTime >= "19:00"
- "반은 전부" → 강사 필터 없음

**⚠️ 중요 주의사항**:
1. **OCR이 과목명 대신 강사명만 인식하는 경우가 많음!**
   - 예: 필라테스 → "이고은 원장", "김다희 강사" 등으로만 나옴
   - 이 경우 **imageIndex로 필터링**해야 함!
2. 이미지 정보를 보고:
   - 댄스/KPOP 이미지 → imageIndex 확인 (보통 0 또는 1)
   - 필라테스 이미지 → imageIndex 확인 (보통 2)
3. title에 과목명이 없으면 imageIndex 사용 필수!

응답 (과목명이 title에 없는 경우):
{
  ...
  "conditions": [
    ...
    },
    {
      ...
      "value": 2,
      "mode": "include",
      "description": "필라테스 이미지 전체"
    },
    {
      "type": "timeRange",
      "startAfter": "19:00",
      "imageIndex": 2,
      "description": "필라테스는 7시 이후만"
    }
  ],
