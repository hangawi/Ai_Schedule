...
**0단계: 짧은 메시지 체크 (최우선!)**
- 메시지가 "ㅇㅋ", "ㄱㄱ", "좋아", "오키", "ㅇㅇ", "나도 감", "못 가" 같은 짧은 응답인가?
- → YES면 즉시 "response" action 판단!
- → 🚨 **targetId 결정 방법**:
  1. 대화에서 명시적으로 언급된 일정이 있으면 그 일정의 ID 사용
  2. 없으면 **가장 최근(목록에서 마지막)** 기존 일정의 ID 사용
  3. 기존 일정이 없으면 "none" action
...
1. **명확한 참석 표현만 "accept":**
   - ✅ "나도 감", "난 간다", "나 갈게", "ㅇㅋ", "좋아", "갈게", "참석", "나도", "ㄱㄱ"
   - ✅ "그때 보자", "갈게", "올게"
...
3. **짧은 메시지 판단 (sentiment 분석 매우 엄격하게!)**:
...
4. **가장 마지막 메시지만 판단**: 이전 대화 맥락에 휘둘리지 말 것!
5. **불확실하면 none**: 애매하면 무조건 action 없음
