...
exports.runScheduleSimulation = async (req, res) => {
   try {
      const { roomId } = req.params;
      const { minHoursPerWeek = 3, numWeeks = 4, currentWeek, ownerFocusTime = 'none' } = req.body;
      const startDate = currentWeek ? new Date(currentWeek) : new Date();
      ...
      const room = await Room.findById(roomId)
        .populate('owner', 'firstName lastName email defaultSchedule scheduleExceptions personalTimes priority')
        .populate('members.user', 'firstName lastName email defaultSchedule scheduleExceptions personalTimes priority')
        .lean(); // Use .lean() for a plain JS object since we are not saving
      ...
      // --- Start of simulation-specific logic ---

      // Create a deep copy of the room to avoid modifying the original object
      const simRoom = JSON.parse(JSON.stringify(room));

      // Filter slots to keep only manually assigned ones (same as runAutoSchedule)
      simRoom.timeSlots = simRoom.timeSlots.filter(slot => {
         if (!slot.assignedBy) return true;
      ...
      if (!simRoom.settings.ownerPreferences) {
        simRoom.settings.ownerPreferences = {};
      }
      simRoom.settings.ownerPreferences.focusTimeType = ownerFocusTime;

      const membersOnly = simRoom.members.filter(m => m.user._id.toString() !== simRoom.owner._id.toString());

      // Validation checks (same as runAutoSchedule)
      if (!simRoom.owner || !simRoom.owner.defaultSchedule || simRoom.owner.defaultSchedule.length === 0) {
        const ownerName = `${simRoom.owner?.firstName || ''} ${simRoom.owner?.lastName || ''}`.trim() || '방장';
        return res.status(400).json({
          msg: `방장(${ownerName})이 선호시간표를 설정하지 않았습니다. 내프로필에서 선호시간표를 설정해주세요.`
        });
      }
      ...
      const ownerBlockedTimes = [];
      const existingCarryOvers = [];
      for (const member of simRoom.members) {
        if (member.carryOver > 0) {
          existingCarryOvers.push({
            memberId: member.user._id.toString(),
            neededHours: member.carryOver,
      ...
      // In simulation, we start with a clean slate for slots and negotiations
      simRoom.timeSlots = [];
      simRoom.negotiations = [];

      // Run the algorithm
      const result = schedulingAlgorithm.runAutoSchedule(
         membersOnly,
         simRoom.owner,
         simRoom.timeSlots, // Pass empty array
         {
            minHoursPerWeek,
            numWeeks,
            currentWeek,
            ownerPreferences: simRoom.settings.ownerPreferences || {},
            roomSettings: {
               ...simRoom.settings,
               ownerBlockedTimes: ownerBlockedTimes
            },
      ...
      // --- Process results without saving ---
      ...
      const simulationResponse = {
        ...simRoom, // Basic room info
        timeSlots: simulatedSlots, // The new simulated slots
        negotiations: result.negotiations || [], // New negotiations
        unassignedMembersInfo: result.unassignedMembersInfo,
        // We don't need to populate again, as we started with populated data
      };

      res.json(simulationResponse);

   } catch (error) {
      console.error('❌ Error running schedule simulation:', error);
      res.status(500).json({ msg: `시뮬레이션 실행 중 오류가 발생했습니다: ${error.message}` });
   }
};