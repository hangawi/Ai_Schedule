...
#### 7-1. AI 채팅 "중복 일정으로 스킵" 오류 수정
- **현상**: AI가 실제로 중복이 아닌 일정도 "중복 일정으로 스킵"으로 처리
- **원인**: `aiScheduleService.js` (162-183줄) `handleNewSchedule()`에서 중복 판단 조건이 너무 느슨함 — **같은 날짜 + 시간 차이 2시간 이내**면 무조건 중복 처리, 제목(summary)을 비교하지 않음
- **예시**: "점심 약속 12:00"과 "스터디 모임 13:00"이 중복으로 처리됨
- **수정 방안**: 같은 날짜 + 같은 시작시간 + 같은 제목일 때만 중복 처리
- **수정 파일**: `server/services/aiScheduleService.js`

#### 7-2. 일정 소유권 이전 (제안자 탈퇴 시)
- **현상**: A가 일정을 만들고 B, C가 참여 → A가 불참(거절) → 삭제 권한이 A에게만 있어서 아무도 삭제 못함
- **수정 방안**:
  - 백엔드 `chatController.js`: `rejectSuggestion`에서 제안자(suggestedBy) 불참 시 suggestedBy를 null로 설정, `deleteSuggestion`에서 suggestedBy가 null이면 accepted 멤버 누구나 삭제 가능
  - 프론트엔드 `SuggestionModal.js`: `isOwner` 함수 수정 — suggestedBy가 null이면 현재 사용자가 accepted 멤버일 때 true 반환
- **수정 파일**: `server/controllers/chatController.js`, `client/src/components/chat/SuggestionModal.js`

#### 7-3. 캘린더 색상 범례 추가
- **현상**: 앱 일정(빨간색)과 구글 캘린더 일정(파란색)의 구분을 사용자가 모름
- **수정 방안**: 캘린더 뷰 상단에 작은 원(dot) + 텍스트로 "앱 일정" / "구글 캘린더" 범례 표시, 구글 연동 사용자에게만 표시
- **수정 파일**: `MobileCalendarView.js`, `MobileCalendarView.css`
...
#### 7-7. extend 시 다른 일정을 "같은 값"으로 오판
...
#### 7-10. 나머지 window.confirm → CustomAlertModal 변환
