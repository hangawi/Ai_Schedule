...
28. **로그아웃을 해도 로그인이 되는 문제** ⏳ 작업 예정
   - 증상: 로그아웃 버튼을 눌러도 로그아웃이 안 되고 다시 로그인된 상태로 돌아옴
   - 원인:
     - 모바일 페이지들(MobileDashboard, MobileScheduleView, MobileGroupsView, MobileCalendarView)의 `handleLogout` 함수가 잘못 구현됨
     - `localStorage.removeItem('token')` 만 하고 Firebase `auth.signOut()` 호출 안 함
     - Firebase 세션이 살아있어서 페이지 리로드 시 자동으로 다시 로그인됨
   - 수정 필요:
     - 모든 모바일 페이지에서 `handleLogout` 수정:
       ```javascript
       const handleLogout = async () => {
          await auth.signOut();                      // ✅ Firebase 로그아웃
          localStorage.removeItem('loginMethod');    // ✅ 올바른 키
          navigate('/auth');                         // ✅ SPA 네비게이션
       };
       ```
     - 또는 `useAuth`의 `handleLogout` prop으로 받아서 사용

29. **페이지 이동 시 로그인 화면 깜빡임 문제** ⏳ 작업 예정
   - 증상: 로그인된 상태에서 버튼을 눌러 페이지를 이동할 때 잠깐 로그인 화면이 보였다가 다시 원래 페이지로 돌아옴
   - 원인:
     - 일부 코드에서 `window.location.href` 사용으로 페이지 전체 리로드
     - 리로드 시 `useAuth`가 처음부터 다시 실행되면서 Firebase 인증 확인하는 동안 `isLoggedIn`이 잠깐 `false`
     - `App.js`의 라우트 보호 로직(`isLoggedIn ? ... : <Navigate to="/auth" />`)이 `/auth`로 리디렉션
     - Firebase 인증 완료 후 다시 원래 페이지로 돌아오면서 깜빡임 발생
   - 수정 필요:
     - 모든 `window.location.href` → `navigate()` 변경
     - 로고 클릭, 메뉴 클릭, 사이드바 메뉴 모두 `navigate()` 사용
     - 페이지 전체 리로드 제거로 상태 유지 및 깜빡임 제거

30. **그룹 화면에서 즉시 특정 방에 들어가는 문제** ⏳ 작업 대기
   - 증상:
     - 원래: 그룹(`/mobile/groups`) 클릭 → 조율방 생성/들어가기 버튼 화면 → 방 선택/생성
     - 지금: 그룹 클릭 → **즉시 어떤 특정 방 안으로 들어가버림**
   - 원인:
     - RoomList.js에서 "새 조율방 생성", "조율방 참여" 버튼 완전 삭제됨 (21번 작업)
     - MobileGroupsView가 CoordinationTab을 사용하는데 자동으로 방에 들어가는 로직 존재
     - isInRoom 상태가 잘못 설정되었거나 초기 상태 관리 문제
   - 수정 필요:
     - RoomList.js에 "새 조율방 생성", "조율방 참여" 버튼 다시 추가
     - MobileGroupsView에서 초기에는 방 목록만 보이도록 수정
     - CoordinationTab의 자동 입장 로직 확인 및 수정

31. **달력 페이지 선호시간/개인시간 디자인 깨짐** ⏳ 작업 대기
   - 증상:
     - 원래 디자인: `선호시간    개인시간` ← 가로로 나란히, 띄어쓰기 정상
     - 현재 디자인:
       ```
       선호시
       간개인시간
       ...
       ← "선호시간"이 줄바꿈되어 "선호시\n간", "개인시간"과 붙어있음
   - 원인:
     - MobileCalendarView.css에 레이아웃 CSS가 정의되지 않음:
       - `.sections-container`
       - `.preference-section`
       - `.personal-section`
       - `.subsection-title`
     - CSS 없이 브라우저 기본 스타일로 렌더링
     - 컨테이너가 좁아서 텍스트 줄바꿈 발생
     - 두 섹션 사이 간격 없음
   - 수정 필요:
     - MobileCalendarView.css에 레이아웃 CSS 추가:
       ```css
       .sections-container {
          display: flex;
          gap: 16px;
