...
### Phase 7: 달력 페이지 UI/UX 전면 개편 (진행 중)
- ✅ 1단계: '일정 관리' 섹션 디자인 개선 및 편집 모드 연동
- ✅ 2단계: 달력 제스처(스와이프) 기본 기능 구현
- ⏳ 3단계: 초기 진입 시 '오늘' 날짜 자동 선택 및 일정 표시 (수정 필요)
- ⏳ 4단계: '자연스러운 스와이프' (다음 달 미리보기 효과) 구현 시도
- ⏳ 5단계: 편집 모드에서만 캘린더 네비게이션 툴바 표시

---

## 🚀 Phase 7 상세 개발 계획 (달력 개편 - 수정됨)

### 1. 일정 관리 섹션 개편 (완료)
- '편집' 버튼 클릭 시에만 하단 관리 섹션 노출.

### 2. 달력 스와이프 UX 고도화 (진행 예정)
- **목표**: 달력을 당길 때 다음 달/이전 달의 내용이 살짝 보이거나 자연스럽게 연결되는 느낌 구현.
- **전략**: FullCalendar DOM 조작 한계상, `transform` 애니메이션과 `transition`을 정교하게 조정하여 '카드 넘기기' 또는 '슬라이드' 효과를 극대화.

### 3. 일간 뷰 일정 자동 표시
- **목표**: 일간 뷰 진입 시, 일정을 클릭하지 않아도 하단에 해당 날짜의 전체 일정 목록이 자동으로 표시됨.
- **수정**: `renderBottomSection` 로직에서 `calendarView === 'timeGridDay'`일 때 무조건 해당 날짜의 `dayEvents` 리스트 렌더링.

### 4. 툴바 조건부 표시
- **목표**: 평소에는 달력만 깔끔하게 표시하고, '편집' 모드에서만 네비게이션(`< >`, `월/주/일`) 버튼 노출.
- **구현**: CSS 클래스 토글링으로 `.fc-header-toolbar`의 `display` 속성 제어.
