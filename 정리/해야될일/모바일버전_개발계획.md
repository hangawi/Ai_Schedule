...
### Phase 7: 달력 페이지 UI/UX 전면 개편 (계획)
- ⏳ 1단계: '일정 관리' 섹션 디자인 개선 및 편집 모드 연동
- ⏳ 2단계: 달력 제스처(스와이프) 이동 기능 구현
- ⏳ 3단계: 초기 진입 시 '오늘' 날짜 자동 선택 및 일정 표시

---

## 🚀 Phase 7 상세 개발 계획 (달력 개편)

### 1. 일정 관리 섹션 개편
- **목표**: 상시 노출되던 선호시간/개인시간 관리 기능을 '편집' 모드에 통합하여 평소에는 깔끔한 달력만 보이게 함.
- **작업 내용**:
  - '일정 관리' 타이틀에 세련된 타이포그래피 및 아이콘 디자인 적용.
  - `isEditing` 상태일 때만 하단 관리 섹션(`SimplifiedScheduleDisplay`)이 나타나도록 조건부 렌더링 수정.

### 2. 달력 스와이프(제스처) 이동
- **목표**: 버튼 클릭 없이 손가락 드래그로 달력을 넘길 수 있는 모바일 친화적 UX 제공.
- **작업 내용**:
  - 터치 이벤트(`onTouchStart`, `onTouchEnd`)를 감지하여 스와이프 방향 판별.
  - **위로 드래그(Swipe Up)**: 이전 달로 이동 (요청 사항 반영).
  - **아래로 드래그(Swipe Down)**: 다음 달로 이동.
  - FullCalendar API(`prev`, `next`)와 연동.

### 3. 초기 진입 및 선택 로직 강화
- **목표**: 달력 탭 진입 시 사용자가 별도 조작 없이 오늘의 일정을 바로 확인할 수 있게 함.
- **작업 내용**:
  - `useEffect`에서 컴포넌트 마운트 시 `selectedDate`를 `new Date()`(오늘)로 강제 설정.
  - 오늘 날짜에 해당하는 일정 리스트가 하단에 즉시 렌더링되도록 보장.
  - 다른 날짜 클릭 시 선택 상태 변경 및 리스트 갱신 로직 유지.
