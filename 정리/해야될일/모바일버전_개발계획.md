...
### 🐛 새로 발견된 문제들 (2026-01-07 - 6차)

36. **모바일 그룹 방 상세 화면 UI 전면 개편** ⏳ 작업 예정
   - 증상:
     - 모바일에서 그룹 방에 입장 시 PC 버전 화면이 그대로 나와 레이아웃이 깨지고 글씨가 너무 큼.
     - 상단에 "그룹", "새 조율방 생성" 등 방 목록 헤더가 중복 표시됨.
     - 채팅창이 없어 일정 조율이 불가능함.
   - 목표:
     - **탭 기반 UI 도입**: 하단 네비게이션으로 [관리 | 멤버 | 요청 | 시간표] 탭 구성.
     - **탭 순서 변경**: 사용 빈도를 고려하여 `관리 > 멤버 > 요청 > 시간표` 순으로 배치.
     - **UI 최적화**: 각 컴포넌트(`Timetable`, `MemberList` 등)의 스타일을 모바일 화면 크기에 맞게 축소.
     - **헤더 정리**: 방 입장 시 상단 "그룹" 헤더를 숨기고 방 전용 헤더만 표시.
     - **채팅 통합**: 하단 탭 또는 플로팅 버튼으로 채팅(AI 비서) 기능 추가.

### 🐛 새로 발견된 문제들 (2026-01-07 - 8차)

38. **방 입장 후에도 목록 헤더(그룹, 생성 버튼)가 사라지지 않는 오류** ⏳ 작업 예정
   - 증상: 방에 들어갔음에도 상단에 "그룹", "새 조율방 생성", "조율방 참여" 버튼이 계속 남아있음.
   - 해결 예정: `MobileGroupsView`에서 방 상태(`isInRoom`)를 더 엄격하게 체크하여 조건부 렌더링 수정.

39. **채팅봇 버튼 클릭 시 화면이 하얗게 변하는 오류 (White Screen)** ⏳ 작업 예정
   - 증상: 플로팅 채팅 버튼을 누르면 화면 전체가 하얗게 변하며 앱이 멈춤 (렌더링 에러 혹은 상태 충돌).
   - 해결 예정: `CoordinationTab`의 채팅 오버레이 로직 및 `ChatBox` 컴포넌트의 에러 바운더리 체크.
