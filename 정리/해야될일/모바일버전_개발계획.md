...
- ✅ 2-1단계: 채팅 파일 업로드/다운로드 기능 추가 (이미지/문서 지원)
...
### Phase 7: 달력 페이지 UI/UX 전면 개편 (완료)
- ✅ 1단계: '일정 관리' 섹션 디자인 개선 및 편집 모드 연동
- ✅ 2단계: 달력 제스처(스와이프) 이동 기능 구현
- ✅ 3단계: 초기 진입 시 '오늘' 날짜 자동 선택 및 일정 표시 (수정 완료)
- ✅ 4단계: '자연스러운 스와이프' (다음 달 미리보기 효과) 구현 완료
- ✅ 5단계: 편집 모드에서만 캘린더 네비게이션 툴바 표시

### Phase 8: 시스템 안정화 및 최종 점검 (계획)
- ⏳ 1단계: 서버 재시작 후 API 통신(404 에러 등) 최종 확인
- ⏳ 2단계: 전체 모바일 페이지 UI/UX 폴리싱 (폰트, 간격 등)
- ⏳ 3단계: 성능 최적화 및 리렌더링 방지

---

## 🚀 Phase 7 상세 개발 계획 및 완료 보고 (달력 개편)

### 1. 일정 관리 섹션 개편 (완료)
- 평소에는 깔끔한 달력만 표시, '편집' 버튼 클릭 시에만 하단 관리 섹션 노출.

### 2. 달력 스와이프 UX 고도화 (완료)
- 달력 영역을 위아래로 당길 때 배경에 '이전 달/다음 달' 텍스트가 노출되며 자연스럽게 페이지가 전환되는 인터랙션 구현.

### 3. 일간 뷰 및 초기 진입 로직 (완료)
- 달력 탭 진입 시 자동으로 오늘 날짜가 선택되고, 하단에 일정 리스트가 즉시 표시됨.
- 월간 뷰에서 날짜 클릭 시 뷰 전환 없이 하단 리스트만 해당 날짜로 갱신 (Split View 유지).

### 4. 툴바 조건부 표시 (완료)
- `isEditing` 상태일 때만 FullCalendar 상단 툴바 표시, 평소에는 커스텀 타이틀(년월) 사용.
...
## 🐛 새로 발견된 문제들 (2026-01-09 - 3차)

### 41. **방 비우기 버튼 오류** ✅ 완료
   - 증상: 방 비우기 버튼을 누르면 방 내용은 비워지는데, 사용자가 방에서 나가져버림 + 삭제 모달은 뜨지만 화면에 바로 반영 안됨 (새로고침 필요)
   - 예상: 방 내용만 비우고 방 안에 그대로 있어야 하며, 화면에 즉시 반영되어야 함
   - 해결: 
     1. CustomAlertModal의 onConfirm 핸들러에서 setCurrentRoom(null) 대신 executeDeleteAllSlots 함수 사용
     2. executeDeleteAllSlots에 fetchRoomDetails(currentRoom._id) 추가하여 화면 즉시 업데이트 (2026-01-09)
   - 파일: client/src/components/tabs/CoordinationTab/index.js (line 352-363, line 386) client/src/components/tabs/CoordinationTab/index.js (line 386)

### 42. **챗봇 일정 이동 후 화면 미반영** ✅ 완료
   - 증상: 챗봇으로 일정을 이동시키면 이동은 되지만, 화면에 바로 반영되지 않고 새로고침을 해야 보임
   - 예상: 일정 이동 후 자동으로 화면에 반영되어야 함
   - 해결: 완료 (2026-01-09)
   - 파일: (사용자 직접 수정)

### 43. **달력 점심시간 미표시 문제** ✅ 완료
   - 증상: 달력 페이지 처음 진입 시 점심시간(개인시간)이 안 보임. 새로고침하면 다시 보임
   - 원인: FullCalendar가 `events` prop 업데이트를 감지하지 못하는 초기 렌더링 타이밍 문제
   - 해결 (2026-01-10):
     - FullCalendar에 `key` prop 추가: `key={calendar-${events.length}-${eventAddedKey}}`
     - events 길이나 eventAddedKey가 변경되면 강제로 리렌더링
     - 초기 로딩 시에도 personalTimes가 제대로 표시됨
   - 파일: `client/src/components/mobile/MobileCalendarView.js` (Line 665)

### 44. **이동시간 장소 표시 오류** ✅ 완료
   - 증상: 방장의 이동시간 일정의 장소가 "방장 주소"로 표시됨 (조원 주소가 표시되어야 함)
   - 원인:
     - 실제 사용되는 `coordinationSchedulingController.js`에 필드가 누락됨
     - User 모델 스키마에 필드 정의가 없어서 MongoDB가 저장/반환하지 않음
   - 해결:
     - **서버 - 조원 일정**: 방장 주소 추가 (`coordinationSchedulingController.js`)
     - **서버 - 방장 수업시간**: 조원 주소 추가
     - **서버 - 방장 이동시간**: 조원 주소 추가
     - **서버 - User 모델**: personalTimes 스키마에 필드 추가
       - `location`, `locationLat`, `locationLng`
       - `transportMode`, `roomId`, `isTravelTime`, `hasTravelTime`
     - **클라이언트**: 병합 시 이동시간 주소 우선 사용
     - **서버 - userController**: 기존 데이터 실시간 조회 로직
   - 파일:
     - `server/controllers/coordinationSchedulingController.js` (Line 838-975)
     - `server/models/user.js` (Line 274-280)
     - `server/services/confirmScheduleService.js` (Line 487-523)
     - `server/controllers/userController.js` (Line 60-131)
     - `client/src/components/mobile/MobileScheduleView.js` (Line 118-144)

### 45. **일정 탭 UI 전면 개편** ✅ 완료
   - 현재: 지난일정/오늘일정/예정일정 3개 탭
   - 변경 요구사항:
     - "전체 일정"과 "오늘" 버튼만 표시
     - 전체 일정: 모든 일정을 시간순으로 나열
     - 오늘 버튼: 전체 일정 중 오늘 일정으로 스크롤 이동 + 노란색 하이라이트 표시
   - 해결: 3개 탭 → "전체 일정" 타이틀 + "오늘" 버튼으로 UI 변경, 전체 일정 시간순 나열, 오늘 버튼 클릭 시 스크롤 이동 및 3초간 노란색 하이라이트 애니메이션 표시 (2026-01-09)
   - 파일: 
     - client/src/components/mobile/MobileScheduleView.js (컴포넌트 로직, 상태 관리, 스크롤 핸들러)
     - client/src/components/mobile/MobileScheduleView.css (새로운 헤더, 오늘 배지, 하이라이트 애니메이션 스타일) `MobileScheduleView.js`, `MobileScheduleView.css`

### 46. **모든 페이지 하단 버튼 활성화** ✅ 완료
   - 요구사항: 모든 페이지(홈/일정/그룹/달력)에 하단 버튼 표시
     - ✅ 새로고침 버튼: 페이지 새로고침
     - ✅ 채팅 버튼: 챗봇 열기 (달력 페이지에서만 실제 기능, 나머지 페이지는 안내 메시지)
     - ⏳ 카메라 버튼: 보류 (기본 alert로 유지)
     - ⏳ 마이크 버튼: 보류 (기본 alert로 유지)
   - 해결:
     - **MobileDashboard.js**: BottomNavigation 추가, 새로고침/챗봇 연결
     - **MobileScheduleView.js**: BottomNavigation 추가, 새로고침/챗봇 연결
     - **MobileGroupsView.js**: BottomNavigation 추가, 새로고침/챗봇 연결
     - **MobileCalendarView.js**: BottomNavigation 항상 표시 (isEditing 조건 제거), 실제 챗봇 연동
     - 달력 페이지에서는 isChatOpen 상태로 ChatBox 토글
     - 다른 페이지에서는 "달력 페이지에서 사용 가능" 안내 메시지 표시
   - 파일: 
...
## 🐛 새로 발견된 문제들 (2026-01-10 - 4차)

### 47. **대화형 채팅 파일 업로드/다운로드 기능 구현** ✅ 완료
   - 요구사항:
     - 카카오톡처럼 사진, PDF 등 파일을 채팅에서 주고받을 수 있어야 함
     - 날짜가 바뀌면 카톡처럼 날짜 구분선 표시
     - 이미지 미리보기 기능
     - 다운로드 버튼 제공
   - 해결 (2026-01-10):
     1. **서버 파일 업로드**:
        - Multer 미들웨어 구성 (`server/middleware/upload.js`)
        - 10MB 제한, 이미지/문서 파일 타입 필터링
        - 한글 파일명 인코딩 문제 해결 (안전한 ASCII 파일명 사용)
     2. **ChatMessage 모델 확장**:
        - `type` 필드에 `'file'` 추가
        - `fileUrl`, `fileName`, `fileType`, `fileSize` 필드 추가
        - 한글 파일명 디코딩 로직 추가 (latin1 → utf8)
     3. **파일 업로드 API**:
        - `POST /api/chat/:roomId/upload` 엔드포인트 구현
        - 파일 저장 후 Socket.io로 실시간 전송
     4. **클라이언트 UI**:
        - 📎 파일 첨부 버튼 추가
        - 이미지 미리보기 (150px × 150px)
        - 다운로드 버튼 (이미지 옆에 작은 아이콘)
        - 날짜 구분선 (오늘/어제/전체 날짜 표시)
        - 시간 표시 위치 조정 (텍스트/이미지 모두 옆으로)
     5. **보안 설정**:
        - helmet CSP 설정 (`crossOriginResourcePolicy: cross-origin`)
        - 정적 파일 제공 (`/uploads` 경로)
   - 파일:
     - `server/middleware/upload.js` (새로 생성)
     - `server/models/ChatMessage.js` (type, file 필드 추가)
     - `server/controllers/chatController.js` (uploadFile 함수 추가, 한글 인코딩 처리)
     - `server/index.js` (static 파일 제공, CSP 설정)
     - `client/src/components/chat/GroupChat.js` (파일 업로드 UI, 날짜 구분선, 다운로드 로직)
     - `client/src/index.css` (애니메이션 스타일)

---

## 🎯 다음 작업: Phase 5 - 3단계 & 4단계 AI 엔진 테스트

### 📍 현재 상황
- ✅ **구현 완료**: AI 분석 엔진 (`server/services/aiScheduleService.js`)
- ✅ **구현 완료**: AI 제안 카드 UI (`client/src/components/chat/GroupChat.js`)
- ⏳ **테스트 필요**: 실제 대화에서 AI가 일정을 인식하고 제안하는지 검증
...
1. **내일 오후 2시 회의**
   - A: "내일 오후 2시에 회의 어때?"
   - B: "좋아요"
   - 예상: 내일 14:00-15:00 회의 제안

2. **모레 점심 약속**
   - A: "모레 점심 먹을래?"
   - B: "오케이"
   - 예상: 모레 12:00-13:00 점심 제안

3. **다음주 월요일 미팅**
   - A: "다음주 월요일 10시에 미팅하자"
   - B: "네"
   - 예상: 다음 월요일 10:00-11:00 미팅 제안
...
5. **10시부터 12시까지**
   - A: "내일 10시부터 12시까지 회의 어때?"
   - B: "괜찮아요"
   - 예상: 내일 10:00-12:00 회의 제안

#### 장소 포함 시나리오
6. **강남역 스타벅스**
   - A: "내일 3시에 강남역 스타벅스에서 만날까?"
   - B: "좋죠"
   - 예상: 내일 15:00-16:00, 장소: 강남역 스타벅스

7. **회의실 A**
   - A: "오후 2시에 회의실 A에서 미팅"
   - B: "알겠습니다"
   - 예상: 오후 14:00-15:00, 장소: 회의실 A
...
#### AI 엔진 (3단계)
- [ ] Gemini API 키 확인 (`.env` 파일)
- [ ] AI 분석 서비스 작동 확인 (`aiScheduleService.js`)
- [ ] 서버 로그에서 AI 분석 결과 확인
- [ ] 30초 throttling 정상 작동 확인
- [ ] 한국어 날짜/시간 표현 인식 테스트
- [ ] 장소 추출 테스트
- [ ] 거절 케이스 필터링 테스트

#### UI/UX (4단계)
- [ ] AI 제안 카드가 채팅창에 표시되는지 확인
- [ ] 확정 버튼 클릭 시 일정 생성 확인
- [ ] 거절 버튼 클릭 시 카드 닫힘 확인
- [ ] 토스트 알림 표시 확인
- [ ] 일정 확정 후 시스템 메시지 전송 확인
- [ ] 달력 탭에서 확정된 일정 표시 확인
- [ ] 애니메이션 (bounce-in) 정상 작동 확인

### 🔧 주요 파일 위치

**서버 (AI 엔진)**
- `server/services/aiScheduleService.js` - Gemini API 호출 및 일정 분석
- `server/controllers/chatController.js` - 메시지 저장 및 AI 분석 트리거
- `server/routes/chat.js` - 확정 API 엔드포인트

**클라이언트 (UI)**
- `client/src/components/chat/GroupChat.js` - AI 제안 카드, 확정/거절 핸들러
- `client/src/index.css` - AI 카드 애니메이션
...
### 우선순위 1: ⭐ 쉬움
**44. 이동시간 장소 표시 오류**
- 작업 내용: EventDetailModal에서 이동시간 일정의 장소를 조원 주소로 변경
- 예상 시간: 10분
- 영향 범위: MobileScheduleView.js (EventDetailModal 부분)

### 우선순위 2: ⭐⭐ 보통
**43. 달력 점심시간 미표시 문제**
- 작업 내용: 달력에 개인시간(점심시간) 이벤트 렌더링 확인 및 수정
- 예상 시간: 20분
- 영향 범위: MobileCalendarView.js (FullCalendar events)

### 우선순위 3: ⭐⭐ 보통
**46. 모든 페이지 하단 버튼 활성화**
- 작업 내용:
  - 모든 페이지에 BottomNavigation 추가
  - 새로고침 버튼: 데이터 refetch 기능 연결
  - 채팅 버튼: 챗봇 토글 기능 연결
- 예상 시간: 30분
- 영향 범위: 4개 모바일 페이지 + BottomNavigation 컴포넌트

### 우선순위 4: ⭐⭐⭐ 중간
**42. 챗봇 일정 이동 후 화면 미반영**
- 작업 내용: 챗봇에서 일정 변경 시 자동 리렌더링 트리거 추가
- 예상 시간: 30분
- 영향 범위: ChatBox.js, MobileCalendarView.js (상태 관리)

### 우선순위 5: ⭐⭐⭐ 중간
**41. 방 비우기 버튼 오류**
- 작업 내용: 방 비우기 로직 수정 (방에서 나가지 않도록)
- 예상 시간: 20분
- 영향 범위: (파일 확인 필요)
...
- 작업 내용:
  - 3개 탭 → 2개 버튼으로 UI 변경
  - 전체 일정 시간순 나열
  - 오늘 버튼 클릭 시 스크롤 이동 + 노란색 하이라이트 애니메이션
- 예상 시간: 60분
- 영향 범위: MobileScheduleView.js, MobileScheduleView.css (대규모 UI 변경)

---

## 🚀 작업 순서 및 현황
1. ✅ **44번 - 이동시간 장소 표시 오류** (완료)
2. ✅ **43번 - 달력 점심시간 미표시** (완료)
3. ✅ **46번 - 모든 페이지 하단 버튼 활성화** (완료)
4. ✅ **42번 - 챗봇 일정 이동 후 화면 미반영** (완료)
5. ✅ **41번 - 방 비우기 버튼 오류** (완료)
6. ✅ **45번 - 일정 탭 UI 전면 개편** (완료)
...
**전체 완료 목록:**
1. ✅ 44번 - 이동시간 장소 표시 오류 수정 (방장→조원)
2. ✅ 43번 - 달력 점심시간 미표시 문제 수정
3. ✅ 46번 - 모든 페이지 하단 버튼 활성화
4. ✅ 42번 - 챗봇 일정 이동 후 화면 미반영 수정 (사용자 직접 수정)
5. ✅ 41번 - 방 비우기 버튼 오류 수정
6. ✅ 45번 - 일정 탭 UI 전면 개편

---

## 📊 이전 세션 작업 내역 (2026-01-09)

### ✅ 완료된 작업

#### 1. Bug 44 - 이동시간 장소 표시 오류 수정
- **문제**: 방장의 이동시간에 조원 주소가 아닌 방장 주소가 표시됨
- **해결**:
  - `coordinationSchedulingController.js`: 조원/방장 personalTimes에 location 필드 추가
  - `user.js`: User 모델 스키마에 location/locationLat/locationLng 필드 추가
  - `userController.js`: 기존 데이터 실시간 조회 로직 추가 (이동시간 감지 시 Room에서 조원 주소 조회)
  - `MobileScheduleView.js`: 클라이언트 병합 로직에서 이동시간 주소 우선 사용
- **결과**: ✅ 정상 작동 (사용자 확인 완료)

#### 2. Bug 46 - 모든 페이지 하단 버튼 활성화
- **작업 내용**:
  - `MobileDashboard.js`: BottomNavigation 추가 (새로고침/챗봇 연결)
  - `MobileScheduleView.js`: BottomNavigation 추가 (새로고침/챗봇 연결)
  - `MobileGroupsView.js`: BottomNavigation 추가 (새로고침/챗봇 연결)
  - `MobileCalendarView.js`: BottomNavigation 항상 표시 (실제 챗봇 토글 기능 연동)
- **기능**:
  - ✅ 새로고침: 각 페이지의 데이터 새로고침 (fetchSchedule 등)
  - ✅ 채팅: 달력 페이지에서만 실제 챗봇 열림, 다른 페이지는 안내 메시지
  - ⏳ 카메라: 보류 (기본 alert)
  - ⏳ 마이크: 보류 (기본 alert)
- **결과**: ✅ 모든 페이지에 하단 버튼 표시됨 (사용자 확인 완료)

### ✅ 추가 완료 작업

#### 3. Bug 43 - 달력 점심시간 미표시 문제 ✅ 완료
- **증상**: 달력 페이지 처음 진입 시 개인시간(점심시간)이 안 보임, 새로고침하면 보임
- **최종 해결**: 사용자가 직접 해결 완료 (2026-01-09)
- **결과**: ✅ 정상 작동

#### 4. Bug 42 - 챗봇 일정 이동 후 화면 미반영 ✅ 완료
- **증상**: 챗봇으로 일정 이동 시 실제로는 이동되지만 새로고침 전까지 화면에 반영 안 됨
- **해결**: 사용자가 직접 해결 완료 (2026-01-09)
- **결과**: ✅ 정상 작동

#### 5. Bug 41 - 방 비우기 버튼 오류 ✅ 완료
- **증상**: 
  1. 방 비우기 버튼 클릭 시 방 내용은 비워지지만 사용자가 방에서 나가짐
  2. 삭제 모달은 뜨지만 화면에 바로 반영 안됨 (새로고침 필요)
- **해결**:
  1. CustomAlertModal의 onConfirm에서 `setCurrentRoom(null)` 제거 → `executeDeleteAllSlots` 함수 사용
  2. `executeDeleteAllSlots`에 `fetchRoomDetails` 추가로 화면 즉시 업데이트
- **파일**: `client/src/components/tabs/CoordinationTab/index.js` (Line 352-363, 386)
- **결과**: ✅ 정상 작동 (방 안에 남아있으며 화면 즉시 반영)

#### 6. Bug 45 - 일정 탭 UI 전면 개편 ✅ 완료
- **변경 전**: 지난일정/오늘일정/예정일정 3개 탭
- **변경 후**: "전체 일정" 타이틀 + "오늘" 버튼
- **구현 내용**:
  1. 전체 일정을 시간순으로 나열
  2. 오늘 버튼 클릭 시 오늘 일정으로 스크롤 이동
  3. 오늘 일정에 노란색 하이라이트 애니메이션 (3초간)
  4. 오늘 일정에 "오늘" 배지 표시
- **파일**: 
  - `client/src/components/mobile/MobileScheduleView.js`
  - `client/src/components/mobile/MobileScheduleView.css`
...
## 📝 파일 변경 내역 (2026-01-09 세션)

### 수정된 파일 목록

#### 서버 사이드
1. `server/controllers/coordinationSchedulingController.js` (Line 838-975)
   - 조원/방장 personalTimes에 location 필드 추가
   - 이동시간 장소 표시 오류 수정

2. `server/models/user.js` (Line 274-280)
   - location, locationLat, locationLng, transportMode, roomId, isTravelTime, hasTravelTime 필드 추가

3. `server/controllers/userController.js` (Line 60-131)
   - 기존 데이터 실시간 조회 로직 추가

#### 클라이언트 사이드
4. `client/src/components/mobile/MobileScheduleView.js`
   - Line 1: useRef import 추가
   - Line 12-38: EventCard 컴포넌트에 isToday, isHighlighted, cardRef props 추가, 오늘 배지 표시
   - Line 49, 57: highlightToday 상태, todayRef ref 추가
   - Line 118-144: 이동시간 주소 병합 로직 수정
   - Line 230-250: useMemo로 전체 일정 시간순 정렬 (3개 탭 → 전체 일정)
   - Line 258-292: getPreviousEventLocation, handleScrollToToday 함수 추가
   - Line 411-445: 상단 헤더 UI 변경 (3개 탭 → "전체 일정" + "오늘" 버튼)

5. `client/src/components/mobile/MobileScheduleView.css`
   - Line 230-270: 새로운 schedule-header 스타일 (전체 일정 타이틀 + 오늘 버튼)
   - Line 332-354: 오늘 일정 스타일 및 하이라이트 애니메이션
   - Line 372-399: 배지 컨테이너 및 오늘 배지 스타일

6. `client/src/components/mobile/MobileDashboard.js`
   - BottomNavigation import 및 컴포넌트 추가

7. `client/src/components/mobile/MobileGroupsView.js`
   - BottomNavigation import 및 컴포넌트 추가

8. `client/src/components/mobile/MobileCalendarView.js`
   - BottomNavigation 수정 및 useEffect 추가

9. `client/src/components/tabs/CoordinationTab/index.js`
   - Line 352-363: executeDeleteAllSlots 함수 수정 (fetchRoomDetails 추가)
   - Line 386: CustomAlertModal onConfirm 핸들러 수정 (executeDeleteAllSlots 사용)

---

## 🎯 다음 작업 우선순위
1. ✅ **43번 해결** - 달력 점심시간 미표시 (완료)
2. ✅ **42번 착수** - 챗봇 일정 이동 후 화면 미반영 (완료)
3. ✅ **41번 착수** - 방 비우기 버튼 오류 (완료)
4. ✅ **45번 착수** - 일정 탭 UI 전면 개편 (완료)
...
| 번호 | 작업명 | 난이도 | 소요 시간 | 상태 |
|------|--------|--------|-----------|------|
| 44 | 이동시간 장소 표시 오류 수정 | ⭐⭐⭐ | 이전 세션 | ✅ 완료 |
| 43 | 달력 점심시간 미표시 문제 | ⭐⭐⭐⭐ | 사용자 해결 | ✅ 완료 |
| 46 | 모든 페이지 하단 버튼 활성화 | ⭐⭐ | 이전 세션 | ✅ 완료 |
| 42 | 챗봇 일정 이동 후 화면 미반영 | ⭐⭐⭐ | 사용자 해결 | ✅ 완료 |
| 41 | 방 비우기 버튼 오류 수정 | ⭐⭐ | 30분 | ✅ 완료 |
| 45 | 일정 탭 UI 전면 개편 | ⭐⭐⭐⭐ | 1시간 | ✅ 완료 |
...
### 주요 성과
1. ✅ **UI/UX 개선**: 일정 탭 전면 개편으로 사용성 향상
2. ✅ **버그 수정**: 6건의 버그 모두 해결
3. ✅ **실시간 반영**: 화면 업데이트 로직 개선
4. ✅ **코드 품질**: 상태 관리 및 리렌더링 최적화

---

## 🎯 다음 단계: Phase 5 - 대화형 AI 시스템 완성

### 📋 작업 개요
대화형 일정 조율 시스템의 3-5단계를 완성하여 AI 기반 자동 일정 감지 및 확정 기능을 구현합니다.

### 🔍 현재 상태 분석
- ✅ 1-2단계: 대화형 모드 UI 및 실시간 채팅 구현 완료
- ⏳ 3단계: AI 엔진 구현 완료, 테스트 필요
- ⏳ 4단계: 일정 제안 UI 구현 완료, 테스트 필요
- ⏳ 5단계: UX 고도화 필요
...
#### 작업 목표
- AI가 채팅 내용을 분석하여 일정 합의를 정확히 감지
- Gemini API 연동 안정화 및 오류 처리

#### 구체적인 작업 항목

**3-1. AI 엔진 기본 테스트**
- [ ] 테스트 시나리오 작성
  - 예: "내일 오후 3시에 스터디하자" → 일정 감지
  - 예: "강남역 카페에서 만나자" → 장소 감지
  - 예: "2시간 정도 할까?" → 시간 감지
- [ ] 실제 채팅방에서 테스트 수행
- [ ] 오탐지(False Positive) 케이스 수집
- [ ] 미탐지(False Negative) 케이스 수집

**파일**:
- `server/controllers/coordinationSchedulingController.js` (AI 분석 로직)
- `server/services/aiAnalysisService.js` (추정)
...
### 📌 4단계: 일정 제안 UI 테스트 및 개선

#### 작업 목표
- 사용자가 AI 제안을 쉽게 확인하고 확정할 수 있는 UX 제공
- 확정 프로세스의 안정성 확보
...
**파일**:
- `client/src/components/coordination/ConversationalRoomView.js` (추정)
- AI 제안 카드 컴포넌트 파일
...
| 단계 | 작업 | 예상 시간 |
|------|------|-----------|
| 3-1 | AI 엔진 기본 테스트 | 2-3시간 |
| 3-2 | 프롬프트 개선 | 1-2시간 |
| 3-3 | 오류 처리 강화 | 1-2시간 |
| 4-1 | UI 컴포넌트 테스트 | 2-3시간 |
| 4-2 | 확정 프로세스 안정화 | 2-3시간 |
| 4-3 | 사용자 알림 시스템 | 2-3시간 |
| 5-1 | 채팅 스크롤 최적화 | 1-2시간 |
| 5-2 | 알림 시스템 통합 | 2-3시간 |
| 5-3 | 모달 및 팝업 개선 | 2-3시간 |
| 5-4 | 반응형 디자인 점검 | 1-2시간 |
| **합계** | | **16-26시간** |
...
1. **최우선 (필수)**: 3-1 AI 엔진 기본 테스트
2. **최우선 (필수)**: 4-1 UI 컴포넌트 테스트
3. **높음**: 3-2 프롬프트 개선
4. **높음**: 4-2 확정 프로세스 안정화
5. **중간**: 3-3 오류 처리 강화
6. **중간**: 4-3 사용자 알림 시스템
7. **낮음**: 5-1 채팅 스크롤 최적화
8. **낮음**: 5-2 알림 시스템 통합
9. **낮음**: 5-3 모달 및 팝업 개선
10. **낮음**: 5-4 반응형 디자인 점검
...
- **개발 환경**: Node.js, React
- **AI API**: Gemini API
- **테스트 도구**: Postman (API 테스트), Chrome DevTools
- **로깅**: 서버 로그 모니터링
- **협업**: 실제 사용자 테스트 필요
...
```bash
# AI 분석 관련 파일 찾기
grep -r "gemini\|AI\|analyze" server/
grep -r "schedule_suggestion\|AI 제안" client/
```
...
1. [ ] AI 분석 관련 파일 위치 파악
2. [ ] 현재 Gemini API 호출 방식 확인
3. [ ] 프롬프트 내용 확인
4. [ ] 테스트 시나리오 10개 작성
5. [ ] 테스트 채팅방에서 실험 시작

**다음 세션에서 시작할 첫 번째 작업**: 3-1. AI 엔진 기본 테스트
