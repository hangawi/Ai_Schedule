...
#### 3. Bug 43 - 달력 점심시간 미표시 문제
- **증상**: 달력 페이지 처음 진입 시 개인시간(점심시간)이 안 보임, 새로고침하면 보임
- **시도한 해결책**:
  1. FullCalendar에 `key` prop 추가: `key={calendar-${events.length}-${eventAddedKey}}`
  2. personalTimes 상태 변경 감지 useEffect 추가 (events 재계산)
  3. key prop 확장: `key={calendar-${events.length}-${personalTimes.length}-${defaultSchedule.length}-${eventAddedKey}}`
  4. fetchSchedule 완료 시 `setEventAddedKey(prev => prev + 1)` 추가
- **로그 분석**:
  - 데이터는 정상 로드됨: `personalTimes: 7`
  - convertScheduleToEvents 실행됨: `변환된 이벤트: 106`
  - 상태 변경 감지도 작동: `재계산된 이벤트: 106`
  - **문제**: FullCalendar가 리렌더링되지 않음
- **현재 상태**: ⚠️ 미해결 - 추가 디버깅 필요
- **다음 시도**:
  - FullCalendar의 `eventAdd` 콜백 확인
  - `events` prop이 아닌 `eventSources` 사용 고려
  - FullCalendar API의 `addEvent()` 직접 호출 고려
  - 초기 렌더링 타이밍 문제 (isLoading 완료 후 렌더링 지연)

### ⏳ 대기 중인 작업

#### 4. Bug 42 - 챗봇 일정 이동 후 화면 미반영
- **증상**: 챗봇으로 일정 이동 시 실제로는 이동되지만 새로고침 전까지 화면에 반영 안 됨
- **예상 작업**: ChatBox에서 일정 변경 시 자동 리렌더링 트리거 추가

#### 5. Bug 41 - 방 비우기 버튼 오류
- **증상**: 방 비우기 버튼 클릭 시 방 내용은 비워지지만 사용자가 방에서 나가짐
- **예상 동작**: 방 내용만 비우고 방 안에 그대로 남아있어야 함

#### 6. Bug 45 - 일정 탭 UI 전면 개편
- **요구사항**:
  - 현재: 지난일정/오늘일정/예정일정 3개 탭
  - 변경: "전체 일정"과 "오늘" 버튼만 표시
  - 전체 일정: 모든 일정을 시간순으로 나열
  - 오늘 버튼: 전체 일정 중 오늘 일정으로 스크롤 이동 + 노란색 하이라이트
- **예상 난이도**: ⭐⭐⭐⭐ 복잡
...
4. `client/src/components/mobile/MobileScheduleView.js` (Line 118-144)
5. `client/src/components/mobile/MobileDashboard.js` (import, BottomNavigation 추가)
6. `client/src/components/mobile/MobileGroupsView.js` (import, BottomNavigation 추가)
7. `client/src/components/mobile/MobileCalendarView.js` (BottomNavigation 수정, useEffect 추가, key prop 수정, eventAddedKey 증가 로직)
