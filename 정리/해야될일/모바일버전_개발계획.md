...
34. **카카오맵 경로 보기 출발지 로직 수정** ⏳ 작업 예정
   - 문제: 카카오맵 길찾기 링크 생성 시 출발지가 항상 "내 집"으로 고정됨. 연속된 일정(A→B)의 경우 "A 장소"가 출발지가 되어야 함.
   - 해결:
     - `EventDetailModal`에서 "경로 보기" 클릭 시, 해당 날짜의 이전 일정(previous event)을 탐색.
     - 이전 일정이 있으면 `이전 일정 장소(좌표) → 현재 일정 장소(좌표)`로 링크 생성.
     - 이전 일정이 없으면 `내 집(좌표) → 현재 일정 장소(좌표)`로 링크 생성.

35. **일정 상세 시간 표시 개선 (이동/수업 분리)** ⏳ 작업 예정
   - 문제: 이동시간이 포함된 경우 총 시간만 표시되어 구분이 어려움.
   - 해결:
     - `hasTravelTime`이 `true`일 때 시간 표시 형식을 변경.
     - 예: `이동 09:00~10:00 | 수업 10:00~11:00` 형태로 분리하여 렌더링.
