...
- ⏳ 2단계: 대화형 모드 전용 UI 및 실시간 그룹 채팅 구현 (현재 진행 중)
- ⏳ 3단계: 대화 내용 분석 AI 엔진 및 백엔드 로직 구현
- ⏳ 4단계: 일정 자동 감지 알림 및 원클릭 확정 UI 구현
...
### 2단계: 대화형 모드 전용 UI 및 실시간 채팅 (진행 중)
- **목표**: 대화형 모드 방 입장 시 기존 시간표 대신 채팅방을 메인 화면으로 표시.
- **작업 내용**:
  - `ConversationalRoomView.js`: 대화형 방 전용 메인 컴포넌트 생성.
  - `CoordinationTab`: `currentRoom.mode`에 따라 뷰 분기 처리.
  - `GroupChat`: Socket.io 기반의 실시간 다자간 채팅 컴포넌트 구현.
  - 모바일 하단 탭 재구성: [채팅 | 시간표 | 멤버 | 설정] 순서로 변경.

### 3단계: 대화 분석 AI 엔진 (The Brain)
- **목표**: 채팅 로그를 모니터링하여 일정 합의 신호를 감지.
- **작업 내용**:
  - 백엔드 메시지 훅(Hook): 메시지 저장 시 분석 트리거 가동.
  - LLM 프롬프트 엔지니어링: 대화 맥락에서 날짜, 시간, 장소 추출.
  - 일정 감지 시 `schedule_suggestion` 소켓 이벤트 발송.

### 4단계: 일정 피드백 및 확정 UI
- **목표**: AI가 제안한 일정을 사용자가 쉽게 확인하고 확정.
- **작업 내용**:
  - 채팅창 내 '일정 제안 카드' UI 구현 (수락/거절 버튼 포함).
  - 확정 시 `TimeSlot` 자동 생성 및 캘린더 연동.
  - 확정된 일정에 대한 시스템 메시지 전송 ("📅 일정이 확정되었습니다").
